...
if (isset($_POST["clic"]))    // Si le formulaire a été validé
{
    ...
    if (empty($erreur)) {
        ...
        if (count($utilisateur) > 0) {
            // Utilisateur trouvé
            $resultat .= "Bienvenue <span>$pseudo</span> Vous êtes maintenant connecté.";
        } else {
...
<body>
    ...
    <?php
    if (empty($erreur))
        echo "<div class='resultat'><button class='close-btn' onclick='this.parentElement.style.display=\"none\";'>&times;</button>$resultat</div>";
    else
        echo "<div class='erreur'><button class='close-btn' onclick='this.parentElement.style.display=\"none\";'>&times;</button>$erreur</div>";
    ?>
